...
def _limit_message(reason: str) -> str:
    contact = ('Reach Jeremy Coates at JMCoates@protonmail.com or on '
               '<a href="https://www.linkedin.com/in/jeremymcoates/" target="_blank">LinkedIn</a>.')
    if reason == "user":
        return ("**Demo limit reached.** This proof-of-concept caps messages per visitor "
                "to manage API costs.\n\nWant to see more? " + contact)
    return ("**Demo at capacity.** This proof-of-concept has reached its overall usage cap.\n\n"
            + contact)


# ===========================================================================
# Airtable logging — fire-and-forget on a background thread, never blocks UI
# ===========================================================================
def log_to_airtable(user_id: str, role: str, event_type: str,
                    question: str = "", response_length: int = 0):
    def _send():
        try:
            import urllib.request, json as _json
            base_id = os.environ.get("AIRTABLE_BASE_ID", "")
            table   = os.environ.get("AIRTABLE_TABLE_NAME", "ia_logs")
...
def _build_email_html(summary_text: str) -> str:
    ...
    # Safety net: if parsing produced nothing usable, fall back to the raw
    # text in a readable block rather than sending an empty branded shell.
    _key_fields = ["OUTCOME", "TOPICS_COVERED", "GUIDANCE_PROVIDED", "FOLLOW_UP_INDICATORS"]
    if not any((parsed.get(k) or "").strip() for k in _key_fields):
        safe = (summary_text or "No summary content was generated.").replace(
            "&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        return f"""<!DOCTYPE html><html><body style="margin:0;padding:24px 12px;background:#F4F6F9;">
<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:12px;overflow:hidden;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;">
<tr><td style="background:#1C2333;padding:22px 28px;border-bottom:3px solid #447099;">
<div style="font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#447099;">Lemma · Posit Cloud</div>
<div style="font-size:19px;font-weight:600;color:#E8EDF5;margin-top:2px;">PS Session Summary</div></td></tr>
<tr><td style="padding:20px 28px;"><div style="font-family:monospace;white-space:pre-wrap;font-size:13px;color:#1C2333;line-height:1.6;">{safe}</div></td></tr>
<tr><td style="background:#F4F6F9;padding:16px 28px;border-top:1px solid #E2E6EF;font-size:12px;color:#6B7A99;">Generated by <strong style="color:#1C2333;">Lemma</strong>. Your address was used only to send this summary.</td></tr>
...
def server(input, output, session):
    ...
    # ---- Per-session identity (for rate limiting + logging) ----
    ...
    handoff_generating = reactive.value(False)
    ...
    MAX_HISTORY_EXCHANGES = 10  # rolling window sent to the API per chat turn

    def capped_msgs():
        """
        Rolling window for per-turn chat calls. Keeps the first exchange
        pinned (preserves the user's original goal) plus the most recent
        N exchanges. Handoff and session summaries intentionally still use
        real_msgs() — they run once per session and need the full transcript.
        """
        msgs = real_msgs()
        limit = MAX_HISTORY_EXCHANGES * 2
        if len(msgs) <= limit + 2:
            return msgs
        head = msgs[:2]            # first user message + first assistant reply
        tail = msgs[-limit:]
        # Preserve user/assistant alternation: tail must start on a user turn
    ...
    # All Anthropic calls run off the reactive thread via asyncio.to_thread,
    # so the typing indicator, loading states, and QA progress actually render.
    # Tasks catch their own exceptions and return dicts — .result() never
    # raises anything except Shiny's internal "not ready yet" signal.

    @reactive.extended_task
    async def chat_task(msgs: list, sys_prompt: str) -> dict:
        try:
            resp = await asyncio.to_thread(call_claude, msgs, sys_prompt)
            return {"ok": True, "resp": resp}
        except Exception as ex:
            return {"ok": False, "error": str(ex)}

    @reactive.extended_task
    async def handoff_task(msgs: list, name: str, role: str) -> dict:
        # generate_handoff_summary catches internally and returns error text
        text = await asyncio.to_thread(generate_handoff_summary, msgs, name, role)
        return {"text": text}

    @reactive.extended_task
    async def summary_task(msgs: list, name: str, role: str, session_start: str,
                           esc: bool, handoff_text: str,
                           unresolved: list, feedback: list) -> dict:
        text = await asyncio.to_thread(
            generate_session_summary, msgs, name, role,
    ...
    async def qa_task() -> list:
        def run_all():
            ...
            for test in QA_TESTS:
                try:
                    r = test["fn"]()
                    results.append({"name": test["name"], **r})
                except Exception as ex:
    ...
    # ---- QA tab button (only visible when name matches QA_UNLOCK_NAME env var) ----
    @output
    @render.ui
    def qa_tab_btn_ui():
        unlock = os.environ.get("QA_UNLOCK_NAME", "").strip().lower()
    ...
    def qa_panel_ui():
        ...
        return ui.div({"class": "qa-panel"},
            ...
            ui.div({"class": "qa-progress"},
                "Running all tests — results appear when complete…" if running else
    ...
    @reactive.effect
    def on_qa_result():
        results = qa_task.result()
        with reactive.isolate():
            if not qa_running():
                return
            qa_results.set(results)
    ...
    def handle_demo_launch():
        ...
        ok_rl, reason = check_and_increment(user_id)
        if not ok_rl:
            add_msg("assistant", _limit_message(reason), is_system=True)
            return
        started.set(True)
        ...
        pending_meta.set({"user_text": message, "is_scope_q": False, "role": role})
        log_to_airtable(user_id, role, "demo_launch", message)
        chat_task([{"role": "user", "content": message}], sys_prompt)
    ...
    def _do_escalate():
        ...
        handoff_task(
            ...
            input.customer_role() or "Not specified",
        )
    ...
    def handle_send():
        ...
        # Rate limit — checked after session-end/escalation so a capped user
        # can still close out or escalate to a human
        ok_rl, reason = check_and_increment(user_id)
        if not ok_rl:
            mid = next_mid()
            messages.set(messages() + [{"role": "user", "content": user_text,
        ...
        pending_meta.set({"user_text": user_text, "is_scope_q": is_scope_q, "role": current_role})
        chat_task(capped_msgs(), sys_prompt)
    ...
    def on_chat_result():
        ...
        with reactive.isolate():
            ...
            current_role = meta.get("role", "")

            if not result.get("ok"):
                add_msg("assistant", _friendly_error(result.get("error", "unknown")), is_system=True)
                return

            resp = result["resp"]
            log_to_airtable(user_id, current_role, "chat",
                            meta.get("user_text", ""), len(resp))
            ...
            messages.set(messages() + [{
                ...
                "source_badge": source, "scope_choice": meta.get("is_scope_q", False),
    ...
    def on_summary_result():
        ...
        with reactive.isolate():
            ...
            summary_generating.set(False)
            log_to_airtable(user_id, input.customer_role() or "", "session_end",
...
def parse_summary(raw):
    ...
    def strip_prefix(line):
        # Tolerate leading whitespace, list markers (-, *), and markdown bold/italics
        # around the field label, e.g. "**FOLLOW_UP_INDICATORS:**", "- RISK_ASSESSMENT:"
    ...
    for line in raw.split("\n"):
        ...
        matched = False
        for f in fields:
            # Match "FIELD:" optionally followed by a closing ** from bold labels
            m = re.match(rf'^{f}\s*:\s*\**\s*(.*)$', stripped)
            if m:
                if cur_key:
                    result[cur_key] = "\n".join(cur_val).strip()
                cur_key, cur_val = f, [m.group(1).strip()]
                matched = True
                break
    ...
    if cur_key:
        result[cur_key] = "\n".join(cur_val).strip()
    # Strip any trailing bold markers the model left on values
    for k in result:
        result[k] = re.sub(r'\s*\**\s*$', '', result[k]).strip()
    return result
...
def format_message(text):
    ...
    for line in lines:
        ...
        if m_num or m_bul:
            tag = 'ol' if m_num else 'ul'
            content = (m_num or m_bul).group(1)
            if in_list and list_tag != tag:
                output.append(f'</{list_tag}>'); in_list = False
            if not in_list:
                output.append(f'<{tag}>'); in_list = True; list_tag = tag
            output.append(f'<li>{content}</li>')
        else:
            if in_list: output.append(f'</{list_tag}>'); in_list = False
            output.append(line)
    if in_list:  output.append(f'</{list_tag}>')
